curl -O https://raw.githubusercontent.com/keploy/keploy/main/keploy.sh && source keploy.sh -n docker
...
keploy test -c "docker run -p 9000:9000 --name negt --network keploy-network  node-express-graphql-template-app" -d 50
