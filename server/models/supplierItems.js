...
const SupplierItem = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    item: {
      type: Item,
      sqlJoin: (supplierItemsTable, itemTable, args) => `${itemTable}.id = ${supplierItemsTable}.item_id`
    },
    supplier: {
      type: Supplier,
      sqlJoin: (supplierItemsTable, supplierTable, args) => `${supplierTable}.id = ${supplierItemsTable}.supplier_id`
    }
  })
});
